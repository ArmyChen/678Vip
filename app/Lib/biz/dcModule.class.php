...
public function Export_weebox(){
	   ...
	   /* 基本参数初始化 */
		...
		$conditions .= " and m.print in (0,1,2,3) and m.location_id=".$id." and m.location_id in(" . implode(",", $account_info['location_ids']) . ") ";

		$sql_count = " select count(id) from " . DB_PREFIX . "dc_menu";
	   // $sql = " select id,name,is_effect,cate_id,price,image from " . DB_PREFIX . "dc_menu ";
		$sql = " select m.*,c.name as catename,c.wcategory,c.wlevel from " . DB_PREFIX . "dc_menu m  left join ".DB_PREFIX."dc_supplier_menu_cate c on c.id=m.cate_id";
