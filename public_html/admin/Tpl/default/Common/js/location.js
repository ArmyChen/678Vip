...
$(document).ready(function(){
	...
	$("select[name='supplier_id']").live('change', function(){
		var is_store_pay = $(this).find("option:selected").attr('data');
		if(is_store_pay == 1){
			$(".is_store_payment").css("display", "");
			$("select[name='open_store_payment']").find("option[text='否']").attr("selected",true);
		}else{
			$(".is_store_payment").css("display", "none");
			$("select[name='open_store_payment']").find("option[text='否']").attr("selected",true);
		}
