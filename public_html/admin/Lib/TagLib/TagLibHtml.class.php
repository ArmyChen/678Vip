...
{//类定义开始
...

    {
        ...
        foreach($fields as $wkey => $field) {

            //显示定义的列表字段

            //武林注释  $parseStr   .=  '<td>&nbsp;';
			$parseStr   .=  '<td class="wtd_'.$wkey.'">';
					...
					if(strpos($val,':')) {
						...
						if(count($b)>1) {
							...
							if(count($c)>1) {
								...
								$parseStr .= '<a href="javascript:'.$c[1].'(\'{$'.$name.'.'.$pk.'}\')"><?php if(0== (is_array($'.$name.')?$'.$name.'["status"]:$'.$name.'->status)){ ?>'.$b[1].'<?php } ?></a><a href="javascript:'.$c[0].'({$'.$name.'.'.$pk.'})"><?php if(1== (is_array($'.$name.')?$'.$name.'["status"]:$'.$name.'->status)){ ?>'.$b[0].'<?php } ?></a><br>';

							}else {

								$parseStr .= '<a href="javascript:'.$a[0].'(\'{$'.$name.'.'.$pk.'}\')"><?php if(0== (is_array($'.$name.')?$'.$name.'["status"]:$'.$name.'->status)){ ?>'.$b[1].'<?php } ?><?php if(1== (is_array($'.$name.')?$'.$name.'["status"]:$'.$name.'->status)){ ?>'.$b[0].'<?php } ?></a><br>';
							...
							$parseStr .= '<a href="javascript:'.$a[0].'(\'{$'.$name.'.'.$pk.'}\')">'.$a[1].'</a><br>';
						...
						if(count($array)>2) {
							...
							$parseStr	.= ' <a href="javascript:'.$array[1].'(\'{$'.$name.'.'.$array[0].'}\')">'.$array[2].'</a><br>';

						}else{

							$parseStr .= ' {$'.$name.'.'.$val.'}<br>';
