...
class TopicGroupAction extends CommonAction{
...

	{
		...
		//2017-1-5 NDD STA
		$cate_list = M("TopicGroupCate")->where(array("pid"=>0))->select();
		$this->assign("cate_list",$cate_list);
		//2017-1-5 NDD END
	...
	public function edit() {		
		...
		$cate_list = D("TopicGroupCate")->toFormatTree($cate_list,'name');	
	...
	//select ajax调用数据 2017-1-5 ndd s
	public function selajax(){
		$id=$_POST['id'];
		if($id != 0){
			$count=$_POST['count']+1;
			$flist=M('TopicGroupCate')->where(array('pid'=>$id))->select();
			
			if($flist){
				$html="<select id='fenlei".$count."' class='selected' onchange='selectop(".$count.")'>";
