...
class SupplierLocationImagesAction extends CommonAction {
	...
	$classid = intval($_REQUEST['classid'])?intval($_REQUEST['classid']):0;
	...
	if($classid){
		$sqlstr="images_group_id=$classid and";
	}
	
	$sql="select * from fanwe_supplier_location_images where  $sqlstr supplier_location_id=".intval($_REQUEST['supplier_location_id']) ." order by id desc limit ".$limit;
	...
	public function insert() {
		...
		if($result['status'] == 1)
		{
			...
			$data['images_group_id'] = intval($_REQUEST['classid']);
			$list = $model->add($data);
			if ($list !== false) { //保存成功
