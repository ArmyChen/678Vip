...
class inventoryModule extends KizBaseModule{
    ...
    public function go_down_index()	{
        ...
        $sql="select a.*,c.name as cname from ".DB_PREFIX."cangku_log a left join ".DB_PREFIX."cangku c on a.cid=c.id ".$sqlstr." and type=1 order by a.id desc limit ".$limit;
     ...
     * 入库添加ajax
     */
    public function go_down_add_ajax()
    {
        init_app_page();
        $account_info = $GLOBALS['account_info'];
        $supplier_id = $account_info['supplier_id'];
        ...
        $sqlcheck="select dd_detail from ".DB_PREFIX."cangku_log where slid=$slid and (danjuhao='$psid' or danjuhao='$dhid')";
