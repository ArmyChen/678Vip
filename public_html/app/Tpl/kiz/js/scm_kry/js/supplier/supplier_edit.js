...
function loadFormData(){
	var allSkuData = new Array(),
	    ...
	    supplierCode = $("#supplierCode1").val(),
	    supplierName = $("#supplierName1").val(),
	    supplierCateId = $("#supplierCateId1").val(),
	...
	$("#supplierCateId").val(supplierCateId);
	$("#bond_sku").val($("#checkbox-0").hasClass("checkbox-check")?true:false);
	$("#bond_price").val($("#checkbox-1").hasClass("checkbox-check")?true:false);
	
	if(supplierName=="") {$("#supplierName1").parent().find(".wrong").html("此栏位为必填");return false;}
	if(supplierCateId=="") {$("#supplierCateId1").parent().parent().parent().find(".wrong").html("此栏位为必填");return false;}
...
function doResetType(){
	 $.ajax({
          url: 'scm/supplier/real/type',
          type: "post",
          async: false,
          data: {},
          success: function (res) {
             if(res.length>0){
            	 var taget = $("#supplierCateId"),isChange = true,
            	 oldVal = taget.parent().find("em").html(),
            	 realList='<li>请选择供应商类别</li>',
            	 realOpt='';
            	 
            	 for(var i=0;i<res.length;i++){
            		 if(isChange&&res[i].supplierCateName==oldVal) isChange = false;
            		 realList+='<li>'+res[i].supplierCateName+'</li>';
            		 realOpt+='<option '+(res[i].supplierCateName==oldVal?"selected":"")+' value="'+res[i].id+'">'+res[i].supplierCateName+'</option>';
            	 }
            	 
            	taget.html("").append('<option '+(isChange?"selected":"")+' value="">请选择供应商类别</option>'+realOpt);
             	taget.parent().find("ul").html("").append(realList);
             	if(isChange) taget.parent().find("em").html("请选择供应商类别");
             }
          },
          error: function () {
             //DO ...
