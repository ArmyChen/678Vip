...
var asnSi = {
    ...
    initQueryList : function() {
        ...
        $gridObj.dataGrid({
            ...
            colNames: ['id', '类型', '时间', '入库原因', '入库仓库', '入库金额', '制单人', '保存 / 确认日期', '状态', '状态'],
            colModel: [
                {name: 'id', index: 'id', width: 50, hidden: true},
                {name: 'asnNo', index: 'asnNo', align: "center", width: 120},
                {name: 'sourceOrderNo', index: 'sourceOrderNo', width: 120, align: "center"},
                {name: 'senderName', index: 'senderName', align: "left", width: 90},
                {name: 'warehouseName', index: 'warehouseName', align: "left", width: 90},
                {
                    name: 'amount',
                    index: 'amount',
                    width: 90,
                ...
                {name: 'creatorName', index: 'creatorName', align: "left", width: 70},
                {name: 'updateTime', index: 'updateTime', align: "center", width: 130},
                {name: 'statusName', index: 'status', align: "center", width: 70},
                {name: 'status', index: 'status', width: 210, hidden: true}
            ],
            sortname: 'asnNo',
            pager: "#gridPager",
            showOperate: true,
    ...
    initDetailGrid : function(editable) {
        ...
        $gridObj.dataGrid({
            ...
            rownumbers: true,
            rowNum : 10000,
            colNames: ['skuId', '所属分类', '商品编码', '商品名称(规格)', '单位', '单位', '价格', '入库数', '合计金额', '当前库存', '当前库存(隐藏)', '当前换算率', '标准单位换算率', '定价', '标准单位ID', '标准单位'],
            colModel: [
