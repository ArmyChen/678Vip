...
class goodsModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		output($root);
	}		/**	 * @作者 周龙权	 * @创建时间 2016年12月25日 下午3:31:24	 * @描述 联盟商户列表	 */	public function top_list() {		$root = array ();		if (isDebug ()) {				/* debug模式输出参数 */			$out = array ();			$root ["out"] = $out;			output ( $root );		}			$req = $GLOBALS ['request'];			/* 门店id */		$slid = intval ( $req ['slid'] );			/* 查询数据 */		$sql = "select id,name,image,buy_count from fanwe_dc_menu where location_id = $slid and location_id > 0 ORDER BY buy_count desc LIMIT 20";		$rs = $GLOBALS ['db']->getAll ( $sql );		if ($rs) {			$data_list = array ();			foreach ( $rs as $v ) {				$v ['image'] = get_abs_img_root ( $v ['image'] );				$data_list [] = $v;			}			$root [data] = $data_list;			output ( $root, 0, "成功" );		} else {			output ( "", 1, "没有数据" );		}	}
}
?>