...
class dpModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		require_once APP_ROOT_PATH."system/model/user.php";				/* 得到星级 */		$point = intval($GLOBALS['request']['point']);				$where = "";		if($point == -1){			/* 获取差评 */			$where = "sld.point <= 2";		}else if($point == 1){						/* 好评 */			$where = "sld.point = 5";		}		else{			;		}
		/*获点评数据*/
		$message_re = get_dp_list($limit,$param=array("deal_id"=>$deal_id,"youhui_id"=>$youhui_id,"event_id"=>$event_id,"location_id"=>$location_id,"tag"=>""),$where,"");		
		$data = array();
		foreach($message_re['list'] as $k=>$v){
		    ...
		    $temp_arr['point_percent'] = $v['point_percent']>0?$v['point_percent']:0;		    //$uinfo = load_user($v['user_id']);		    //$temp_arr['user_name'] = $uinfo['user_name'];
		    $temp_arr['point'] = $v['point'];	        $temp_arr['user_name'] = $v['user_name'];	        $temp_arr['nick_name'] = $v['nick_name'];	        $temp_arr['user_avatar'] = get_abs_img_root ( $v ['avatar']);
		    $images = array();
		    $oimages = array();
		    if($v['images']){
		...
		if(count($data)>0)
		{
			$sql = "select count(*) from ".DB_PREFIX."supplier_location_dp sld where  ".$message_re['condition'];			
