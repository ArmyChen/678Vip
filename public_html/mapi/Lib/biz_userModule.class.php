...
class biz_userModule extends MainBaseModule
{
    ...
    public function check_biz_login(){
		...
		//自动登陆
		$account_name=es_cookie::get("account_name");
		$account_password=es_cookie::get("account_password");
	...
	public function dologin(){
	    ...
	    $auto_login = intval($GLOBALS['request']['auto_login']);
...
// 	    }
	    ...
	    if($result['status'])
	    {
	        ...
	        $result = $GLOBALS['db']->getOne("SELECT location_id FROM ".DB_PREFIX."supplier_account_location_link WHERE account_id=".intval($account_info['id']));
            $slid=$result;
			es_session::set('slid',$slid);
			
			es_cookie::set("account_name",$account_name,604800);
			es_cookie::set("account_password",$account_password,604800);
