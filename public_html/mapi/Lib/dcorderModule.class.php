 ...
 */
	...
	//	file_put_contents("pat.txt",$payment_id);
		...
		$dc_type =$GLOBALS['request']['dc_type'];
		...
		*/
		...
		//file_put_contents("del.txt",json_encode($delivery_time));
		if($delivery_time){
...
public function make_order()
{
	...
	$consignee = strim($GLOBALS['request']['consignee']);
    $mobile = strim($GLOBALS['request']['mobile']);
    $sess_id = strim($GLOBALS['request']['sess_id']);
	...
	$dc_type = $GLOBALS['request']['dc_type'];          //$dc_type大等于0为预订方式，不享受促销优惠，-1代表享受促销优惠
	$zpid = $GLOBALS['request']['zpid'];          //$dc_type大等于0为预订方式，不享受促销优惠，-1代表享受促销优惠
...
//		$menu_list=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_menu where id in(".$menu_arr.")");
		$menu_list=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_cart where cart_type =1 and session_id = '".$sess_id."'");
...
//		$menu_list=data_format_idkey($menu_list,$key='id');
		...
		foreach($menu_list as $k=>$v){
			$location_dc_cart['total_data']['total_price']+=$v['unit_price']*$v['num'];
			$location_dc_cart['total_data']['total_count']+=$v['num'];
		}
			
				
		if($dc_type==-1 || $dc_type==-9 ){  //外卖时才检测
				
			$consignee_info=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."dc_consignee where id=".$consignee_id);			
			
			if(!$consignee_info && $dc_type==-1){
			...
			$delivery_time=$this->get_delivery_time($location_id);
			//file_put_contents("del2.txt",json_encode($delivery_time));
			if(!$delivery_time){

				$root['is_return']=1;
				output($root,0,'不在商家配送时间内2');	
			}
        ...
        foreach($menu_list as $kk=>$vv){
            $menu_order=array();
            $menu_order['session_id']=$vv['session_id'];
            $menu_order['user_id']=$vv['user_id'];
            $menu_order['location_id']=$vv['location_id'];
            $menu_order['name']=$vv['name'];
            $menu_order['icon']=$vv['icon'];
            $menu_order['num']=$vv['num'];
            $menu_order['unit_price']=$vv['unit_price'];
            $menu_order['total_price']=$vv['total_price'];
            $menu_order['menu_id']=$vv['menu_id'];
            $menu_order['cart_type']=$vv['cart_type'];
            $menu_order['add_time']=NOW_TIME;
            $menu_order['is_effect']=$vv['is_effect'];
...
//		$rs_list=$GLOBALS['db']->getAll("select rs_item.price ,rs_item.name ,rs_item.id as item_id , rs_time.* from ".DB_PREFIX."dc_rs_item_time as rs_time left join ".DB_PREFIX."dc_rs_item as rs_item on rs_time.item_id=rs_item.id  where rs_time.id in(".$rs_arr.")");
        $rs_list=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_cart where cart_type =0 and session_id = '".$sess_id."'");
//var_dump("select rs_item.price ,rs_item.name ,rs_item.id as item_id , rs_time.* from ".DB_PREFIX."dc_rs_item_time as rs_time left join ".DB_PREFIX."dc_rs_item as rs_item on rs_time.item_id=rs_item.id  where rs_time.id in(".$rs_arr.")");die;
		$rs_list=data_format_idkey($rs_list,$key='id');
		$location_dc_table_cart=array();
		$location_dc_table_cart['total_data']['total_price']=0;
		$location_dc_table_cart['total_data']['total_count']=0;
//		$rs_num=data_format_idkey($rs_num,$key='id');
		...
		foreach($rs_num as $k=>$v){
            $location_dc_table_cart['total_data']['total_price']+=$v['unit_price']*$v['num'];
            $location_dc_table_cart['total_data']['total_count']+=$v['num'];
		}
...
//        output($root,0,$location_dc_cart);
        
		
		//验证购物车是否为空
		if($dc_type==-1 || $dc_type==-9){  //外卖购物车验证
	...
	if(check_save_login()!=LOGIN_STATUS_LOGINED)
	{
		...
		if(count($location_dc_table_cart['cart_list'])==0 && count($location_dc_cart['cart_list'])==0 ){
			...
			if($location_dc_table_cart['total_data']['total_price'] > 0){
				...
				$delivery_time=$this->get_delivery_time($location_id);
				//file_put_contents("del3.txt",json_encode($delivery_time));
				if(!$delivery_time){
				
					$root['is_return']=1;
					output($root,0,'不在商家配送时间内3');
				}
			...
			//2017.2.6 增加商户扫码点餐支付设置
			$dclocation=$GLOBALS['db']->getRow("select dc_allow_ddf,dc_allow_wxf,dc_allow_yzf from ".DB_PREFIX."supplier_location where id=".$location_id);
			...
			{	
				...
				$dc_pay_str='';
				if($dclocation['dc_allow_wxf']==0){
				$dc_pay_str.=' and id !=20';
				}
				if($dclocation['dc_allow_yzf']==0){
				$dc_pay_str.=' and id !=30';
				}				
				//支付列表
				$sql = "select id, class_name as code, logo,name from ".DB_PREFIX."payment where ( online_pay = 2 or online_pay = 4 or online_pay = 5) and is_effect = 1".$dc_pay_str;
			...
			if($dclocation['dc_allow_ddf']==1){				
			$payment_list[]=array(
			"id"=>1,
			"code"=>'cash',
			"logo"=>'/wap/Tpl/main/fanwe/images/dc/timg.jpg',
			"name"=>'消费结束后吧台结算'			
			);
			}
			
			sort($payment_list);
          //  file_put_contents("play.txt",json_encode($payment_list));
...
public function order_done()
{
		...
		*/
		...
		if(count($location_dc_table_cart['cart_list'])==0 && count($location_dc_cart['cart_list'])==0 ){
			...
			if($location_dc_table_cart['total_data']['total_price'] > 0){
				...
				$delivery_time=$this->get_delivery_time($location_id);
				//file_put_contents("del4.txt",json_encode($delivery_time));
				if(!$delivery_time){
				
					$root['is_return']=1;
					output($root,0,'不在商家配送时间内4');
				}
	 ...
	 * @return array 返回配送时间段数组
	 */
	public function get_delivery_time($location_id,$time_span=600,$delivery_time=array('am'=>array("0:00","23:59"))){
	
		$delivery_time_span=array();
		foreach($delivery_time as $delivery_time_item){
			$delivery_time_span[]=$this->get_time_span(to_timespan($delivery_time_item[0]),to_timespan($delivery_time_item[1]),$time_span);
		}
	    //file_put_contents("time1.txt",json_encode($delivery_time_span));
		$delivery_time_new=array();
		foreach($delivery_time_span as $delivery_time_span_row){
		...
		// file_put_contents("time2.txt",json_encode($delivery_time_new));
		$delivery_time_arr=array();
		foreach($delivery_time_new as $delivery_time_row){
			//file_put_contents("time4.txt",NOW_TIME);
			$check_time=NOW_TIME-28800;
			if($check_time  <= $delivery_time_row){
				//	$delivery_time_arr[]= to_date($delivery_time_row,"H:i");
				$delivery_time_arr[]= $delivery_time_row;
			}
		}
		 file_put_contents("time3.txt",json_encode($delivery_time_arr));
		$open_time_info=$GLOBALS['db']->getAll("select * from ".DB_PREFIX."dc_supplier_location_open_time where location_id=".$location_id);
	   ...
	   // file_put_contents("time.txt",json_encode($open_time_arr_new));
		return $open_time_arr_new;
	}
 ...
 */
	...
	public function load_user_consignee($id){
		global_run();
		$zpname=$GLOBALS['request']['zpname'];
		$zpid=$GLOBALS['request']['zpid'];	
		if ($zpid){
		$location_info=$GLOBALS['db']->getRow("select a.xpoint,a.ypoint,b.zpname from ".DB_PREFIX."supplier_location a left join ".DB_PREFIX."dc_zp b on a.id=b.slid where b.zpid=".$zpid);
		$consignee_info['xpoint']=$location_info['xpoint'];
		$consignee_info['ypoint']=$location_info['ypoint'];	
		$consignee_info['api_address']=$location_info['zpname'];	
		$user_info=$GLOBALS['db']->getRow("select user_name,mobile from fanwe_user where id=".intval($GLOBALS['user_info']['id']));
		$consignee_info['consignee']=$user_info['user_name'];
		$consignee_info['address']=$zpname;
		$consignee_info['user_id']=intval($GLOBALS['user_info']['id']);
		$consignee_info['mobile']=$user_info['mobile'];
