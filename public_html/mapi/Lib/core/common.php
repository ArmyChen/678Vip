 ...
 * @param unknown_type $info 当前消息 可选，为空时由客户端取默认提示(默认提示包含成功的默认提示与失败的默认提示)
 */
function woutput($data,$status=1,$info="")
{
	$r_type = intval($_REQUEST['r_type']);//返回数据格式类型; 0:base64;1;json_encode;2:array 3:jsonp
	$data[CTL] = MODULE_NAME;
	$data[ACT] = ACTION_NAME;
...

{
	...
	$deal['icon']=get_abs_img_root(get_spec_image($v['icon'],92,92,1));
...
// 		$images[] = get_abs_img_root(get_spec_image($v['img'],230,140,1));
	    $images[] = get_abs_img_root(get_spec_image($v['img'],230,230,1));
...
/**
 * @作者 周龙权
 * @创建时间 2016年12月25日 下午4:02:15
 * @描述 是否调试模式
 */
function isDebug(){
	if(isset($_REQUEST[debug]) && strim($_REQUEST[debug]) == "678-1134"){
		return true;
	}else{
		return false;
	}
