...

{
	...
	public function index() {
		...
		$focus_user_list = load_auto_cache ( "cache_focus_user", array (
				"uid" => $home_user_info ['id'] 
		) );
		...
		if ($id <= 0) {
			// 当id小于0是查看所有用户的动态
			$condition = " is_effect = 1 and is_delete = 0  and fav_id = 0 and relay_id = 0  and type in ('share','small_video','dealcomment','youhuicomment','eventcomment','slocationcomment','eventsubmit','sharedeal','shareyouhui','shareevent') ";
		} else {
			$condition = " user_id in (" . implode ( ",", $t_ids ) . ") and is_effect = 1 and is_delete = 0  and fav_id = 0 and relay_id = 0  and type in ('share','small_video','dealcomment','youhuicomment','eventcomment','slocationcomment','eventsubmit','sharedeal','shareyouhui','shareevent') ";
		}
		
		$result_list = get_topic_list ( $limit, null, "", $condition );
		...
		if ($app_type == 'app') {
			...
			$expression_replace_array = load_auto_cache ( "expression_replace_array", array (
					'type' => 'url' 
			) );
		...
		foreach ( $result_list ['list'] as $k => $v ) {
			...
			if ($v ['type'] == 'share') {
			...
			} else if ($v ['type'] == 'small_video') {
				$temp_data ['small_video_url'] = $v ['small_video_url'];
			} else {
				...
				if ($v ['type'] == 'dealcomment' || $v ['type'] == 'youhuicomment' || $v ['type'] == 'eventcomment' || $v ['type'] == 'slocationcomment' || $v ['type'] == 'eventsubmit') { // 点评数据
					...
					$temp_data ['share_obj'] ['url'] = SITE_DOMAIN . url ( "index", $v ['rel_route'], array (
							'r' => base64_encode ( $v ['user_id'] ) 
					) );
			...
			$temp_data ['addr_country'] = $v ['addr_country'];
			
			$temp_data ['addr_province'] = $v ['addr_province'];
			
			$temp_data ['addr_city'] = $v ['addr_city'];
			
			$temp_data ['addr_district'] = $v ['addr_district'];
			
			$temp_data ['addr_street'] = $v ['addr_street'];
			
			$temp_data ['addr_str'] = $v ['addr_str'];
			
			$temp_data ['addr_xpoint'] = $v ['addr_xpoint'];
			
			$temp_data ['addr_ypoint'] = $v ['addr_ypoint'];
	...
	public function index2() {
		...
		if ($id) {
...

			{
				...
				$home_user_info = $GLOBALS ['db']->getRow ( "select * from " . DB_PREFIX . "user where id=" . $uid );
		...
		$user_data ['nick_name'] = $home_user_info ['nick_name'];
		
		$user_data ['user_avatar'] = get_abs_img_root ( $home_user_info ["avatar"] );
		
		$user_data ['uc_home_bg'] = get_abs_img_root ( $home_user_info ["home_bg"] );
		...
		/* 当id小于0是查看所有用户的动态 */
		$condition = " t.is_effect = 1 and t.is_delete = 0  and fav_id = 0 and relay_id = 0  and t.type in ('share','small_video','dealcomment','youhuicomment','eventcomment','slocationcomment','eventsubmit','sharedeal','shareyouhui','shareevent') and t.create_time < unix_timestamp(now())";
		 ...
		 */
		if ($id > 0) {
			$condition = " user_id in (" . implode ( ",", $t_ids ) . ") and $condition";
		}
		
		$result_list = get_topic_list ( $limit, null, "left join fanwe_user as fu on t.user_id = fu.id ", $condition );
		
		$count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "topic t where " . $condition );
		 ...
		 *  if ($app_type == 'app') {
			...
			foreach ( $expression_replace_array ['replace'] as $k => $v ) {
				...
				$expression_replace_array ['replace'] [$k] = str_replace ( 'http://www.678sh.com/public/', 'http://oss.678sh.com/wsh/', $v );
		...
		foreach ( $result_list ['list'] as $k => $v ) {
			...
			if (count ( $v ['images'] ) > 0) {
				...
				foreach ( $v ['images'] as $ik => $iv ) {
					...
					$s_img [] = get_abs_img_root ( $iv ['o_path'] );
			...
			$temp_data ['nick_name'] = $v ['nick_name'];
			
			/* $temp_data ['user_avatar'] = get_abs_img_root ( get_muser_avatar ( $v ['user_id'], "big" ) ) ? get_abs_img_root ( get_muser_avatar ( $v ['user_id'], "big" ) ) : ""; */
			
			$temp_data ['user_avatar'] = get_abs_img_root ( $v ['avatar'] );
			...
			/* 点赞列表 */
			$fav_list = get_topic_fav_list($v ['id'],100);
			
			$temp_data ['fav_list'] = $fav_list ? $fav_list : array ();
	...
	public function do_fav_topic() {
		...
		if ($user_login_status != LOGIN_STATUS_LOGINED) {
			...
			if (! $topic) {
				...
				/* 该动态已经不存在 */
				output ( $root, 0, $GLOBALS ['lang'] ['TOPIC_NOT_EXIST'] );
			} else {
				if ($topic ['user_id'] == $user_id) {
					
					/* 自己的动态 */
					output ( $root, 0, $GLOBALS ['lang'] ['TOPIC_SELF'] );
				} else {
					...
					if ($count > 0) {
						...
						/* 已经赞过了 */
						output ( $root, 0, $GLOBALS ['lang'] ['TOPIC_FAVED'] );
					} else {
						require_once APP_ROOT_PATH . "system/model/topic.php";
						...
						output ( $root, 1, $GLOBALS ['lang'] ['FAV_SUCCESS'] );
					}
				}
			}
		}
		...
		if ($user_login_status != LOGIN_STATUS_LOGINED) {
			...
			if (! $topic) {
				...
				if ($topic ['user_id'] == $user_id) {
					...
					$count = $GLOBALS ['db']->getOne ( "select count(*) from " . DB_PREFIX . "topic_fav where topic_id = $id and user_id = $user_id");
					if ($count > 0) {
						...
						require_once APP_ROOT_PATH . "system/model/topic.php";
						$GLOBALS['db']->autoExecute(DB_PREFIX."topic_fav",array("user_id" => $user_id,"topic_id" => $id ));
	...
	public function do_reply() {
...

		{
...

			{
				...
				output ( $root, 0, $GLOBALS ['lang'] ['MESSAGE_SUBMIT_FAST'] );
			}
			...
			$root ['reply_data'] = $GLOBALS ['db']->getRow ( "SELECT ftr.id,ftr.topic_id,ftr.user_id,ftr.user_name,ftr.content,ftr.create_time,ftr.reply_id,ftr.reply_user_id,ftr.reply_user_name,fu.nick_name,fu_r.nick_name as reply_nick_name from " . DB_PREFIX . "topic_reply as ftr LEFT JOIN " . DB_PREFIX . "user AS fu ON ftr.user_id  = fu.id LEFT JOIN " . DB_PREFIX . "user AS fu_r ON ftr.reply_user_id  = fu_r.id where ftr.is_effect=1 and ftr.is_delete=0 and ftr.id=$reply_id" );
		}
	...
	public function focus() {
...

		{
			...
			output ( $root, 0, $GLOBALS ['lang'] ['FOCUS_SELF'] );
		}
		...
		rm_auto_cache ( "cache_focus_user", array (
				"id" => $user_id 
		) );
	...
	public function publish() {
		...
		if ($user_login_status != LOGIN_STATUS_LOGINED) {
			...
			$result = $GLOBALS ['db']->getAll ( "select type,title,emotion,CONCAT('http://oss.678sh.com/wsh/expression/',type,'/',filename) filename from " . DB_PREFIX . "expression where type = 'qq' order by order_num" );
	...
	public function do_publish() {
		...
		if ($user_login_status != LOGIN_STATUS_LOGINED) {
			...
			if (count ( $file ['file'] ['name'] ) > 0) {
				...
				foreach ( $imgs as $url ) {
					$img_id = insert_topic_oss_img ( $url );
					$topic_image = array ();
					$topic_image ['type'] = "image";
					$topic_image ['id'] = $img_id;
					$attach_list [] = $topic_image;
				}
			}
			...
			// 国家
			$addr_country = strim ( $GLOBALS ['request'] ['addr_country'] );
			$addr_province = strim ( $GLOBALS ['request'] ['addr_province'] );
			$addr_city = strim ( $GLOBALS ['request'] ['addr_city'] );
			$addr_district = strim ( $GLOBALS ['request'] ['addr_district'] );
			$addr_street = strim ( $GLOBALS ['request'] ['addr_street'] );
			$addr_str = strim ( $GLOBALS ['request'] ['addr_str'] );
			$addr_xpoint = strim ( $GLOBALS ['request'] ['addr_xpoint'] );
			$addr_ypoint = strim ( $GLOBALS ['request'] ['addr_ypoint'] );
			$small_video_url = strim ( $GLOBALS ['request'] ['small_video_url'] );
			
			// 如果客户端有上传small_video_url值
			if ($small_video_url != null && strlen ( $small_video_url ) > 0) {
				$type = "small_video";
			}
			
			$id = insert_topic ( $content, $title, $type, $group, $relay_id = 0, $fav_id = 0, "", $attach_list, array (), $tags, '', '', '', 0, 0, $addr_country, $addr_province, $addr_city, $addr_district, $addr_street, $addr_str, $addr_xpoint, $addr_ypoint, $small_video_url );
	...
	public function del_topic() {
		...
		if ($user_login_status != LOGIN_STATUS_LOGINED) {
			...
			$req = $GLOBALS ['request'];
			$topId = $req ["id"];
			$sql = "update " . DB_PREFIX . "topic set is_delete = 1 where user_id = $userId and id = $topId";
