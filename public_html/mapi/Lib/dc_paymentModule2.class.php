...
class dc_paymentModule extends MainBaseModule
{
	...
	/*====================================================================================*/
		...
		$channelId = $GLOBALS['db']->getone("select appid from fanwe_app where slid='$location_id' and isdisable=1 order by loginTime desc limit 1 ");
		if ($channelId==false){
		$channelId=$GLOBALS['db']->getone("select appid from fanwe_app where slid='$location_id' order by loginTime desc limit 1");
		} //调取最后登陆的那一位
// message content.
        $tuidata=array(
           'order_id'=>$order_id
