...

{
...

	{
		...
		$result = get_deal_list ( 50, array (
				DEAL_ONLINE,
				DEAL_NOTICE 
		), array (), " left join " . DB_PREFIX . "deal_location_link as l on d.id = l.deal_id ", " d.buy_type <> 1 and d.is_shop = 0 and l.location_id =" . $store_id );
		...
		$result = get_goods_list ( 50, array (
				DEAL_ONLINE,
				DEAL_NOTICE 
		), array (), " left join " . DB_PREFIX . "deal_location_link as l on d.id = l.deal_id ", " d.buy_type <> 1 and d.is_shop = 1 and l.location_id =" . $store_id );
		...
		$result = get_event_list ( 10, array (
				EVENT_NOTICE,
				EVENT_ONLINE 
		), array (), " left join " . DB_PREFIX . "event_location_link as l on e.id = l.event_id ", " l.location_id = " . $store_id );
		...
		$result = get_youhui_list ( 50, array (
				YOUHUI_NOTICE,
				YOUHUI_ONLINE 
		), array (), ' left join ' . DB_PREFIX . "youhui_location_link as l on y.id = l.youhui_id ", " l.location_id = " . $store_id );
		...
		/* 获点评数据 */
		$dp_list = get_dp_list ( 5, $param = array (
		...
		foreach ( $dp_list ['list'] as $k => $v ) {
			...
			$temp_arr ['user_name'] = $v ['user_name'];
			$temp_arr ['nick_name'] = $v ['nick_name'];
			$temp_arr ['user_avatar'] = get_abs_img_root ( $v ['avatar'] );
			// $temp_arr['user_avatar'] = get_muser_avatar ( $v ['user_id'], "big" );
			$images = array ();
			$oimages = array ();
			if ($v ['images']) {
		...
		// 作者：周龙权
		// 日期：2016-12-15 14:14
		$dp_list_img = get_dp_list ( 5, $param = array (
				"location_id" => $store_id 
		), "is_img = 1", "" );
		// output($dp_list_img);
		$format_dp_list_img = array ();
		foreach ( $dp_list_img ['list'] as $k => $v ) {
			$temp_arr = array ();
			$temp_arr ['id'] = $v ['id'];
			$temp_arr ['create_time'] = $v ['create_time'] > 0 ? to_date ( $v ['create_time'], 'Y-m-d' ) : '';
			$temp_arr ['content'] = $v ['content'];
			...
			$temp_arr ['images'] = $images;
			$temp_arr ['oimages'] = $oimages;
			$format_dp_list_img [] = $temp_arr;
		}
		$root ['dp_list_img'] = $format_dp_list_img;
		
		// 得到该门店详细评分
		// 作者：周龙权
		// 日期：2016-12-15 14:14
		$dp_info = load_dp_info ( array (
				"location_id" => $store_id 
		) );
		...
		// 评价星级
		$root ['avg_point'] = $dp_info ["avg_point"];
		
		// 总评论数
		$root ['dp_count'] = intval ( $dp_info ["dp_count"] );
		
		// 各项评分
		$root ['point_group_json'] = json_encode ( $dp_info ["point_group"] );
		
		$root ['page_title'] = $GLOBALS ['m_config'] ['program_title'] ? $GLOBALS ['m_config'] ['program_title'] . " - " : "";
		
		$root ['page_title'] .= "门店点餐";
	 ...
	 * @描述 联盟商户列表
	 */
	public function union_shop_list() {
		$root = array ();
		if (isDebug ()) {
		...
		/* 查询数据 */
		$sql = "select fsxc.xf_slid,fsxc.scoretocash,fsl.name,fsl.address,fsl.preview from fanwe_score_xiaofei_cfg as fsxc left JOIN fanwe_supplier_location as fsl on fsxc.xf_slid = fsl.id where slid = $slid";
