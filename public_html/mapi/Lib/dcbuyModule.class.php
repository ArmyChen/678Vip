            ...
            */
            ...
            foreach($location_menu_cate as $k=>$v){
                ...
                foreach($location_menu_cate[$k]['sub_menu'] as $kk=>$vv){
                    ...
                    $location_menu_cate[$k]['sub_menu'][$kk]['attr_list'] = array();
                    $sql = "select * from ".DB_PREFIX."dc_supplier_taste where shops like '%".$location_menu_cate[$k]['sub_menu'][$kk]['id']."%' and location_id in (".$dclocation['id'].")";
                    $deal_attrs = $GLOBALS['db']->getAll($sql);
                    if(count($deal_attrs) > 0){
                        $location_menu_cate[$k]['sub_menu'][$kk]['has_attrs'] = true;
                    }else{
                        $location_menu_cate[$k]['sub_menu'][$kk]['has_attrs'] = false;
                    }
                    foreach ($deal_attrs as $key=>$item) {
                        $location_menu_cate[$k]['sub_menu'][$kk]['attr_list'][$key]['attr'] = $item;
