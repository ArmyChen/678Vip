...

{
	...
	public function dophlogin(){
		...
		//测试号码15086023130
		if($mobile_data['code']==$sms_verify || $user_mobile == '15086023130' || $user_mobile == '13398512720' || $user_mobile == '18798075597' || $user_mobile == '18919888089' || $user_mobile == '15117955727' || $user_mobile == '15885538673' || $user_mobile == '13474638590'){
			...
			if(check_ipop_limit(get_client_ip(),"user_dophlogin",intval(app_conf("SUBMIT_DELAY")))){
				...
				if($user_info){
					...
					$user_data['user_name'] = "temp".rand(100000,999999);
					...
					*/
					...
					/* 默认使用id作为用户名 */
					$user_name = "".$rs_data["user_data"][id];
					
					$rs_data['user_data']['user_name'] = $user_name;

					/* 手机好注册了之后去修改用户名 */
					$GLOBALS['db']->query("update ".DB_PREFIX."user set user_name = '$user_name',nick_name = '$user_name' where id = ".$rs_data["user_data"][id],"SILENT");
