...
if($op == 'post') {	
	...
	$fanwe_waiter=pdo_fetchall("select * from fanwe_waiter where slid=:slid",array("slid"=>$slid));
...
if($op == 'submit') {
	...
	$order = array(
		...
		'data_json' => json_encode($cart['data']),
		'note' => trim($_GPC['pay_type2']).'@@@'.trim($_GPC['note'])
	);
